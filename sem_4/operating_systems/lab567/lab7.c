...
bool fillFromFile(LineTable* lt, char* map, off_t size) {
	char buf[READ_BUFFER + 1];

	size_t lastPush = 0;

	for (off_t idx = 0; idx < size; idx++) {
		if (map[idx] == '\n') {
			bool ok = pushLine(lt, idx);
			lastPush = idx;
			if (!ok) {
	...
	if (size - lastPush > 0) {
		bool ok = pushLine(lt, lastPush);
		if (!ok) {
			perror("Error while writing last line offset. Failed allocation?...");
			return false;
		}
	}
...
size_t readFromTable(LineTable* lt, char* map, size_t line,
	...
	*/
	...
	while (1) {
		...
		if (lt->top <= ln - 1) {
			...
			size_t ok = readFromTable(lt, map, ln - 1, buf, MAX_PRINT);
