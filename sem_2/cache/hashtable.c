...
htTable_t* newHashtable(size_t cap, void dtor(void*)) {
	if (cap == 0) { // default
		cap = 32;
	}
...
static void htResize(htTable_t* htbl, size_t newSize) {
	if (newSize < htbl->curStored) {
		printf("Can't resize to %zu! (want: %zu, stored elements: %zu)\n", newSize, newSize, htbl->curStored);
		return;
	}
...
void htFreeElement(htTable_t* tbl, htElem_t* elem) {
	if (tbl->dtor != NULL) {
		tbl->dtor(elem->data);
	}
	free(elem->key);
